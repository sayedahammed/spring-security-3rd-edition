package com.security.calender.dataaccess;
...
public class JdbcEventDao implements EventDao {
    ...
    public int createEvent(final Event event) {
        ...
        final Calendar when = event.getWhenTime();
        if(when == null) {
        ...
        this.jdbcOperations.update(new PreparedStatementCreator() {
            public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
                PreparedStatement ps = connection.prepareStatement(
                        "insert into events (when_time,summary,description,owner,attendee) values (?, ?, ?, ?, ?)",
                        new String[] { "id" });
    ...
    private static final RowMapper<Event> EVENT_ROW_MAPPER = new RowMapper<Event>() {
        public Event mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            when.setTime(rs.getDate("events.when_time"));
    ...
    private static final String EVENT_QUERY = "select e.id, e.summary, e.description, e.when_time, " +
